...
def verticalbar(cv19):
  ...
  def make_plot(src, origin):
    p = figure(plot_width = 800, plot_height = 600, x_axis_label='Date', x_axis_type='datetime', y_axis_label='Deaths', title=f'Total Deaths on {origin}')
    p.vbar(x='date', top='total_deaths', bottom=0, source=src, color='firebrick')

    hover = HoverTool(tooltips=[('Country', '@location'), ('Total Deaths', '@total_deaths'), ('Date', '@date{%Y-%m-%d}')], formatters={'@date' :'datetime'},  line_policy='next')
    p.add_tools(hover)
    return p
  ...
  def update(attr, old, new):
    ...
    p.title.text=f'Total Deaths on {origin}'
  ...
  tab = Panel(child=layout, title='Vertical bar(New Deaths)')
