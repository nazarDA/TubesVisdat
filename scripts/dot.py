...
def dotdot(cv19):
  ...
  def make_plot(src, origin):
    p = figure(plot_width = 800, plot_height = 600,
               x_axis_label='New cases', y_axis_label='New deaths', title=f'New Cases and New Deaths Comparison on {origin}')
    p.circle('new_cases', 'new_deaths', source=src, color='firebrick', line_width=2)

    hover = HoverTool(tooltips=[('Country', '@location'), ('New cases', '@new_cases'), ('New Deaths', '@new_deaths')],  line_policy='next')
    p.add_tools(hover)
    return p
  ...
  def update(attr, old, new):
    ...
    p.title.text=f'New Cases and New Deaths Comparison on {origin}'
  ...
  tab = Panel(child=layout, title='Circle plot')
  return tab